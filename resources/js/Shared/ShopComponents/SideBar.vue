<template>
  ...
  >
    ...
    <div
      ...
      :class="{
        '-translate-y-0 opacity-100 block mt-3 md:block': dropdown,
        'opacity-0 md:opacity-100 md:-translate-y-0 -translate-y-10 hidden md:block': !dropdown,
      }"
    >
      <div>
        ...
        <div class="space-y-2">
          <CategoryCheckbox
            ...
            @checked="(ischecked) => checkedCat(ischecked, category)"
          ></CategoryCheckbox>
        </div>
      </div>
...
export default {
  data() {
    return {
      selectedCat: new Set(),
      oldSelectedCat:
        typeof this.$page.props.query.categories !== "undefined"
  ...
  methods: {
    ...
    sendRequest() {
      this.selectedCat.size > 0
        ? (this.searchQuery.categories = JSON.stringify([...this.selectedCat]))
        : false;
