            ...
            <TabGroup as="div" class="mt-2">
              <div class="border-b border-gray-200">
                <TabList class="-mb-px flex space-x-8 px-4">
                  <Tab as="template" v-for="category in navMenu.categories" :key="category.name" v-slot="{ selected }">
                    <button :class="[
                      selected
              ...
              <TabPanels as="template">
                <TabPanel v-for="category in navMenu.categories" :key="category.name"
                  ...
                  <div class="grid grid-cols-2 gap-x-4">
                    <div v-for="item in category.featured" :key="item.name" class="group relative text-sm">
                      ...
                      <Link :href="item.href" class="mt-6 block font-medium text-gray-900">
                  ...
                  <div v-for="section in category.sections" :key="section.name">
                    ...
                    <ul role="list" :aria-labelledby="`${category.id}-${section.id}-heading-mobile`"
                      ...
                      <li v-for="item in section.items" :key="item.name" @click="open = false" class="flow-root">
                        <Link :href="item.link" class="-m-2 block p-2 text-gray-500" preser>{{ item.name }}</Link>
                      </li>
                    </ul>
                  </div>
                </TabPanel>
        ...
        <div class="flex h-16 items-center">
          ...
          <PopoverGroup class="hidden lg:ml-8 lg:block lg:self-stretch z-50">
            <div class="flex h-full space-x-8 items-center">
              <Popover v-for="category in navMenu.categories" :key="category.name" class="flex" v-slot="{ open }">
                ...
                <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0"
                  ...
                  <PopoverPanel class="absolute inset-x-0 top-full text-sm text-gray-500" v-slot="{ close }">
                        ...
                        <div class="grid grid-cols-2 gap-x-8 gap-y-10 py-10">
                          <div class="col-start-2 grid grid-cols-2 gap-x-8">
                            <div v-for="item in category.featured" :key="item.name"
                              ...
                              <div class="overflow-hidden rounded-lg bg-gray-100 group-hover:opacity-75 h-48 w-48">
                                <img :src="item.imageSrc" :alt="item.imageAlt" @click="close"
                                  class="object-cover object-center" />
                              </div>
                              <Link :href="item.href" @click="close" class="mt-6 block font-medium text-gray-900">
                              <span class="absolute inset-0 z-10" aria-hidden="true" />
                              {{ item.name }}
                              </Link>
                              <p aria-hidden="true" class="mt-1">Shop now</p>
                          ...
                          <div class="row-start-1 grid grid-cols-3 gap-x-8 text-sm">
                            <div v-for="section in category.sections" :key="section.name">
                              ...
                              <ul role="list" :aria-labelledby="`${section.name}-heading`"
                                ...
                                <li v-for="item in section.items" :key="item.name" class="gap-y-4">
                                  <Link :href="item.link" class="hover:text-gray-800" @click="close">
                                  <p>{{ item.name }}</p>
                                  </Link>
                                </li>
                              </ul>
              ...
              <Link v-for="page in navMenu.pages" :key="page.name" :href="page.href"
                class="flex items-center text-sm font-medium text-gray-700 hover:text-gray-800">{{ page.name }}</Link>
            </div>
          </PopoverGroup>
          ...
          <div class="ml-auto flex items-center">
            ...
            <div id="dropdownSearch" class="z-10 bg-gray-400 rounded-lg shadow w-60 dark:bg-gray-700 mt-10"
              ...
              <div class="p-3">
                ...
                <div class="relative">
                  ...
                  <input type="text" id="input-group-search" v-model="searchTerm"
            ...
            <div class="ml-4 flow-root lg:ml-6">
              <div class="group -m-2 flex items-center p-2">
                <ShoppingBagIcon class="h-6 w-6 flex-shrink-0 text-gray-400 group-hover:text-gray-500 cursor-pointer"
